...
class Reporter {
    ...
    report(message, { line = 1, col = 1 }, rule) {
        if (
            line in this.allow && rule.id in this.allow[line]
...
const CSSLint = (() => {
    ...
    return Object.assign(new parserlib.util.EventTarget(), {
        ...
        verify(text, ruleset = this.getRuleSet()) {
            const allow = {};
            const ignore = [];
            const emi = rxEmbedded.lastIndex = text.lastIndexOf(
                '/*',
                text.indexOf('csslint', text.indexOf('/*') + 1 || text.length) + 1,
            );
            if (emi >= 0) {
...
CSSLint.addRule['floats'] = [{
    ...
    parser.addListener('property', ({ property, value }) => {
        count += CSSLint.Util.getPropName(property) === 'float'
...
CSSLint.addRule['globals-in-document'] = [{
    ...
    const check = (event) => {
        if (level && index) {
            reporter.report(
                `A nested @${event.type} is valid only if this @-moz-document section `
                    + 'is the first one matched for any given URL.',
                event,
                rule,
            );
        }
    ...
    CSSLint.Util.registerRuleEvents(parser, {
        ...
        property({ inParens, value: { parts: [p] } }) {
            ...
            if (miss && (k = Object.keys(miss))[0]) {
                reporter.report(`Missing ${k.join(',')} prefix${k[1] ? 'es' : ''} for gradient.`, miss[k[0]], rule);
            }
            miss = stack.pop();
        },
    });
...
CSSLint.addRule['ids'] = [{
    ...
    parser.addListener('startrule', (event) => {
        for (const sel of event.selectors) {
            const cnt = sel.parts.reduce((sum = 0, { modifiers }) =>
                modifiers
        ...
        for (const { modifiers } of parts || []) {
            ...
            for (const mod of modifiers) {
                if (mod.type === 'pseudo') {
                    ...
                    for (
                        const err of !def ? ['Unknown pseudo'] : [
                            colons.length > 1
                                ? !(def & 2) && 'Must use : in'
    ...
    CSSLint.Util.registerShorthandEvents(parser, {
        property(event, props, name) {
            ...
            if (ovr) {
                delete props[name];
                reporter.report(
                    `"${event.property}" overrides "${Object.keys(ovr).join('" + "')}" above.`,
                    event,
                    rule,
                );
            }
