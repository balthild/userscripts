...
const Properties = {
    ...
    '-webkit-appearance': 'auto | none | button | button-bevel | caps-lock-indicator | caret | checkbox | '
...
for (
    const [k, reps] of Object.entries({
        'border': '{1,4}',
        'border-bottom': '',
        'border-left': '',
        'border-right': '',
...
const VTComplex = {
    ...
    '<bg-layer>': '<bg-image> || <bg-position> [ / <bg-size> ]? || <repeat-style> || '
        + '<attachment> || <box>{1,2}',
    '<bg-position>': '[ center | [ left | right ] <len-pct>? ] && [ center | [ top | bottom ] <len-pct>? ] | '
    ...
    '<content-list>': '[ <string> | <image> | <attr> | '
    ...
    '<dasharray>': (M) =>
        M.alt([M.term('<len-pct0+>'), M.term('<num0+>')])
    ...
    '<position>': '[ [ left | right ] <len-pct> ] && [ [ top | bottom ] <len-pct> ] | '
...
class StringSource {
    ...
    readMatchStr(m) {
        ...
        if (
            !len || str.charCodeAt(i) === m.charCodeAt(0) && (
                    len === 1
...
class Matcher {
    ...
    static many(req, ms) {
        ...
        m.req = req === true
            ? Array(ms.length).fill(true)
            : req == null
            ? ms.map((m) => !m.arg || m.arg.marker !== '?')
            : req;
        return m;
    }
    /**
    ...
    static parse(str) {
        ...
        if (!source.eof()) {
            ...
            throw new Error(
                `Internal grammar error. Unexpected "${
                    clipString(string.slice(i, 31), 30)
                }" at position ${i} in "${string}".`,
            );
        }
        ...
        do {
            ...
            if (pt) {
                ...
                do {
                    ...
                    do {
                        const seq = [];
                        do seq.push(Matcher.parseTerm(src)); while (src.readMatch(rxSeqSep));
...
function validateProperty(tok, value, stream, Props) {
    ...
    do spec = Props[prop] || Props['<all>'] && (Props = Properties)[prop]; while (
        !spec && !res && (vp = tok.vendorPos) && (res = prop = prop.slice(vp))
    );
