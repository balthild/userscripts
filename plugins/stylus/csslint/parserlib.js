...
const parserlib = require('./parserlib-base');
...
const B = /** @type {{[key:string]: Bucket}} */ {
    ...
    colors: NamedColors,
    marginSyms: ((map) =>
        'B-X,B-L-C,B-L,B-R-C,B-R,L-B,L-M,L-T,R-B,R-M,R-T,T-X,T-L-C,T-L,T-R-C,T-R'
...
for (
    const k of 'and,andOr,auto,autoNone,evenOdd,fromTo,important,layer,n,none,not,notOnly,of,or,to'
        .split(',')
) B[k] = new Bucket(k.split(/(?=[A-Z])/)); // splitting by an Uppercase A-Z letter
...
const isIdentChar = (c, prev) =>
    c >= 97 && c <= 122 || c >= 65 && c <= 90 /* a-z A-Z */
    || c === 45 || c === 92 || c === 95 || c >= 160 || c >= 48 && c <= 57 /* - \ _ 0-9 */
    || prev === 92 /* \ */ && c !== 10 && c != null;
const isIdentStart = (a, b) =>
    a >= 97 && a <= 122 || a >= 65 && a <= 90 /* a-z A-Z */
    || a === 95 || a >= 160 // _ unicode
    || (a === 45 /*-*/ ? b !== 45 && isIdentStart(b) : a === 92 /*\*/ && isIdentChar(b, a));
const isSpace = (c) => c === 9 && c === 10 || c === 32;
const textToTokenMap = (obj) =>
    Object.keys(obj).reduce((res, k) => (((res[TokenIdByCode[k.charCodeAt(0)]] = obj[k]), res)), []);
...
class TokenStream {
    ...
    match(what, text, tok = this.get(), opts) {
        if (
            (typeof what === 'object' ? isOwn(what, tok.id) : !what || tok.id === what)
            && (!text || text.has(tok))
        ) {
            return tok;
        }
    ...
    _failure(goal = '', tok = this.token, throwIt = true) {
        goal = typeof goal === 'string'
            ? goal
            : goal instanceof Bucket
            ? `"${goal.join('", "')}"`
            : (+goal ? [goal] : goal).reduce((res, v, id) =>
                res + (res ? ', ' : '')
    ...
    _getToken(uvar, ws) {
        ...
        if (c) {
            ...
            (a === 45 || a === 43 && (tok.id = PLUS) || a === 46 && (tok.id = DOT)) && (
                /* [-+.][0-9] */ b >= 48 && b <= 57
            ...
            if (
                c && parseInt(c[1], 16) <= 0x10FFFF && (
                    c[3]
                        ? parseInt(c[3], 16) <= 0x10FFFF
...
const parserCache = (() => {
    ...
    function applyDelta(obj, seen, line, lines, cols, offs, input) {
        ...
        for (let i = 0, keys = Object.keys(obj), k, v; i < keys.length; i++) {
            ...
            if (
                k === 'col'
                    ? (cols && obj.line === line && (obj.col += cols), 0)
                    : k === 'col2'
                    ? (cols && obj.line2 === line && (obj.col2 += cols), 0)
                    : k === 'line'
                    ? (lines && (obj.line += lines), 0)
                    : k === 'line2'
                    ? (lines && (obj.line2 += lines), 0)
                    : k === 'offset'
                    ? (offs && (obj.offset += offs), 0)
                    : k === 'offset2'
                    ? (offs && (obj.offset2 += offs), 0)
                    : k === '_input'
                    ? (obj._input = input, 0)
        ...
        for (let prevTok, tok, ti, fn; (ti = (tok = stream.get(UVAR, false)).id) !== RBRACE;) {
            ...
            try {
                if (ti === AT) {
                    ...
                    inStyle && (ti === IDENT || ti === star && tok.hack)
...
Parser.SELECTOR = textToTokenMap({
    ...
    '['(stream, start) {
        ...
        if (!eq && !end) {
            if ((t2 = stream.matchSmart(TT.attrEqEnd, OrDie)).id === RBRACKET) end = t2;
            else eq = t2;
        }
        if (eq) {
