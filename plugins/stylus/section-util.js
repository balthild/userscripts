...
const MozDocMapper = {
    ...
    styleToCss(style) {
        ...
        for (const section of style.sections) {
            const funcs = [];
            MozDocMapper.forEachProp(
                section,
                (type, value) => funcs.push(`${type}("${value.replace(/[\\"]/g, '\\$&')}")`),
            );
            res.push(
                funcs.length
...
module.exports = { calcStyleDigest, MozDocMapper, styleCodeEmpty, styleJSONseemsValid, styleSectionsEqual };
